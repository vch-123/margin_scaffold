...
namespace EdgeSideProgramScaffold.Client
{
    internal class UDPClient
    {
        ...
        public async Task SendBackendStateData(byte[] data)
        {
            // 解析PLC的IP地址。
            IPAddress ipAddress = IPAddress.Parse(_configService.PLC_IP);
            // 获取PLC控制端口。
            int port = _configService.PLC_State_Port;
            // 创建IPEndPoint实例，表示目标网络端点。
            IPEndPoint endPoint = new IPEndPoint(ipAddress, port);
            // 异步发送数据到指定的网络端点。
            _udpClient?.SendAsync(data, data.Length, endPoint);
            // 记录发送数据的日志。
            Log.Debug($"udp<BACKEND> 向{_configService.PLC_IP}:{_configService.PLC_State_Port}发送:{BitConverter.ToString(data)}" + "大小为" + data.Count());
        }    
    }
}
